...
export class AIService { 
    static async generateQuestions(skills, role, count = 10, provider = 'openai') { 
    console.log(`Generating ${count} questions for ${role} with skills: ${skills.join(', ')}`);
    
    // For now, directly use fallback questions due to API quota limits
    // This ensures consistent question generation without API dependency
    ...
    */
    ...
    try {
      const response = await fetch(`https://hooks.jdoodle.net/proxy?url=${encodeURIComponent(AI_ENDPOINTS.openai)}`, { 
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          ...
          messages: [{
            role: 'system',
            content: `Create a personalized learning roadmap for transitioning to ${targetRole}.
